    ...
    Anole.NS('dom').extend((function () {
        ...
        */
        ...
        function getChilds(el, tag) {
            var childs = el.childNodes, results = [];
            if (!childs) return results;
            for (var i = 0; i < childs.length; i++) {
                if (tag) {
                    if (childs[i].nodeType === 1 && childs[i].nodeName === tag) results.push(childs[i]);
                } else {
                    if (childs[i].nodeType === 1) results.push(childs[i]);
                }
            }
            return results;
        }
        function childTotal(el, tag) {
            var childs = el.childNodes, total = 0;
            if (!childs) return total;
            for (var i = 0; i < childs.length; i++) {
                if (tag) {
                    if (childs[i].nodeType === 1 && childs[i].nodeName === tag) total++;
                } else {
                    if (childs[i].nodeType === 1) total++;
                }
            }
            return total;
        }
        ...
        var PSEUDO = {
            ...
            'only-child': function (results, _results, val) {
                ...
                for (var i = 0; i < results.length; i++) {
                    ...
                    if (total === 1) _results.push(results[i]);
                }
            },
            ...
            'nth-last-of-type': function (results, _results, val) {
                var brothers, val = parseInt(val), pos;
                if (isNaN(val)) return;
                val--;
                for (var i = 0; i < results.length; i++) {
                    brothers = getChilds(results[i].parentNode, results[i].nodeName);
